...
function drawSprite(obj, pos) {
  ...
  cxa.beginPath();
  cxa.moveTo(
    x + points[0].x * size.w * xs * units,
    y + points[0].y * size.h * units
  );
  for (var k = 1; k < points.length; k++) {
    cxa.lineTo(
      x + points[k].x * size.w * xs * units,
      y + points[k].y * size.h * units
    );
  }
  cxa.closePath();
  cxa.fill();
...
function drawArrows(obj) {
  ...
  if (
    obj.ArrowAlpha > 0 &&
    interactable &&
    !(selectedController.IsPressed && selectedController !== obj)
  ) {
    var mode = obj.Mode;
    ...
    if (obj.Slider) {
      ...
      var ceilX = comparison(
        obj.ThreeDest.x,
        obj.Slider.origin.x + obj.Slider.range.x
      );
      var floorY = comparison(obj.ThreeDest.y, obj.Slider.origin.y);
      var ceilY = comparison(
        obj.ThreeDest.y,
        obj.Slider.origin.y + obj.Slider.range.y
      );
    }
...
function drawPassage() {
  if (passageAlpha.A > 0) {
    ...
    for (i = 0; i < passageParticles.length; i++) {
      ...
      cxa.fillRect(
        x - su,
        y - (1 + (Player[9].volume.value + 20) * 2) * su,
        2 * su,
        (2 + (Player[9].volume.value + 20) * 4) * su
      );
    }
    cxa.globalAlpha = 1;
  }
