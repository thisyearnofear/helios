...
function mousePress() {
  ...
  if (interactable) {
    ...
    // Show Zora embed when clicking mint text
    if (linkOver[1]) {
      document.getElementById("zora-embed").classList.add("visible");
    }
  }
}
...
function rolloverCheck() {
  ...
  if (scene > 0) {
    for (var i = 0; i < controllers.length; i++) {
      ...
      controller.RollOver = hitBox(
        pos.x - controller.Size.w * 0.5 * units,
        pos.y - controller.Size.h * 0.5 * units,
        controller.Size.w * units,
        controller.Size.h * units
      );
    ...
    closeOver = hitBox(
      halfX - 60 * units,
      panelPos.y + halfY + 90 * units,
      120 * units,
      30 * units
    );
    linkOver[0] = hitBox(
      halfX - 240 * units,
      panelPos.y + halfY - 45 * units,
      160 * units,
      30 * units
    );
    linkOver[1] = hitBox(
    ...
    if (interactable) {
      ...
      if (
        panelOpen &&
        (linkOver[0] || linkOver[1] || linkOver[2] || linkOver[3])
      ) {
        pointer = true;
      }
...
function drag3D() {
  if (mouseIsDown) {
    ...
    rotateDest.x =
      downRotation.x + (Math.PI / 180) * ((mouseX - downPoint.x) / 10);
    rotateDest.y =
      downRotation.y + (Math.PI / 180) * ((mouseY - downPoint.y) / 10);
...
function dragController(controller, toCursor) {
  if (controller.Slider) {
    ...
    if (toCursor) {
      ...
      var posDif = new Point(
        selectedControllerPos.x - mouseDown3D.x,
        selectedControllerPos.y - mouseDown3D.y
      );
...
// Add function to close Zora embed when clicking outside
document.addEventListener("click", function (e) {
  if (e.target.closest("#zora-embed")) return;
  document.getElementById("zora-embed").classList.remove("visible");
});
