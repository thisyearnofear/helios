...
SliderFunctions = [
  ...
  function (v, t, g) {
    if (Math.round(v) !== slideCurrent) {
      ...
      Osc[2].frequency.rampTo(
        Tone.Master.noteToFrequency("" + slideScale[Math.round(v)]),
        0.002
      ); // 8
      Osc[3].frequency.rampTo(
        Tone.Master.noteToFrequency("" + slideScale2[Math.round(v)]),
        0.002
      );
    }
    if (g) {
...
function createController(controller) {
  ...
  threeController.position.set(
    controller.positions[0].x,
    controller.positions[0].y,
    controller.positions[0].z
  );
...
function createBackground(background) {
  ...
  World3D.add(threeController);
  threeController.position.set(
    background.positions[0].x,
    background.positions[0].y,
    background.positions[0].z
  );
...
function createScenery(scenery) {
  ...
  World3D.add(threeController);
  threeController.position.set(
    scenery.positions[0].x,
    scenery.positions[0].y,
    scenery.positions[0].z
  );
...
function createFlickerParticles() {
  for (var i = 0; i < 8; i++) {
    var p = new Particle(
      new Point(-halfX + Math.random() * 200, -halfY + Math.random() * 200),
      new Vector()
    );
    p.Active = true;
...
function createFlickerParticles2() {
  for (var i = 0; i < 14; i++) {
    var p = new Particle(
      new Point(-150 + Math.random() * 300, -halfY + Math.random() * fullY),
      new Vector(0, 10 + Math.random() * 20)
    );
    p.Active = true;
...
function createWorms() {
  for (var i = 0; i < 2; i++) {
    ...
    for (var j = 0; j < 4; j++) {
      var p = new Particle(
        new Point(-30 + Math.random() * 60, -30 + Math.random() * 60),
        new Vector()
      );
      pa.push(p);
    }
...
function createWind() {
  for (var i = 0; i < 4; i++) {
    ...
    for (var j = 0; j < 10; j++) {
      var p = new Particle(
        new Point(
          halfX + Math.random() * fullX,
          -halfY + Math.random() * fullY
        ),
        new Vector()
      );
      pa.push(p);
...
function createDust() {
  for (var i = 0; i < 14; i++) {
    var p = new Particle(
      randomPoint(),
      new Vector(-(10 + Math.random() * 10), -3 + Math.random() * 6)
    );
